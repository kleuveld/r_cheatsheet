...
Loops have the reputation of being slow, so if performance is importatn
you can also use 
`map()` to repeat a function for every element in a vector. 

You can iterate over the vector of items, which is essentially
the same thing I did with a loop above (but faster):
...
But you can also iterate over the rows in a data frame using `pmap()`,
which may be easier to include in a pipe:
...
read_csv(here("data/SAFI_clean.csv"), na = "NULL") %>%
  bind_cols(.,
    pmap_dfr({.}, 
     function(items_owned,...) {
       as_tibble_row(str_count(items_owned,items),
                     .name_repair = ~paste0("owns_",items))
     })
)
...
If writing a vector with possible values is too tedious/error prone:
