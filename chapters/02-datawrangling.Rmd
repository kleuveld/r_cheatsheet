...
interviews %>%
    ...
    mutate(owns_bicycle = str_count(interviews$items_owned,"bicycle")) %>%
    mutate(owns_television = str_count(interviews$items_owned,"television")) 
    #etc etc.
...
for (i in items){
    varname <- i
    interviews2 <-
        mutate(interviews2, {{i}} := str_count(interviews2$items_owned,i))
}
interviews2

```

Note how we used `{{i}} := ` to dynamically create variable names in the `mutate()` function.

Writing a vector with possible values is still tedious, so to prevent even that:

    - split the string variable using `separate_wider_delim()`;
    - pivot to a long list of  all items owned; and then
    - pivot back to household level in a usuable format.
...
```
You can use a `left_join()` to merge this back into the main data set.
