...
items <- c("bicycle","television","solar_panel","table","cow_cart","radio",
           "cow_plough","solar_torch","mobile_phone","motor_cyle","fridge",
...
you can also use 
`map()` to repeat a [function](#functions) for every element in a vector ([here's an 
excellent tutorial for `map()`](https://jennybc.github.io/purrr-tutorial/index.html). 
`map()` returns a list
...
split_items_owned <- function(item) {
    str_count(split_with_map$items_owned,item)
}

items %>%
  map(split_items_owned)%>%
  head(n = 2)
...
A list isn't really great in this case. Fortunately, there's
also `map_df()`, which returns its items as columns in a dataframe. For this
to work, I make sure the vector has names, which will then become
the column names. Also, defining a function to use it 
only once seems like a waste, so I use an anonyous function, defined within 
the `map_df()` call itself: 
...
items %>%
  ...
  map_df(function(x) str_count(split_with_map$items_owned,x)) 
...
list is a row. This also gives you access to all variables in the data set, 
which might be needed for more complex rowwise operations:

```{r eval=TRUE, include = TRUE, echo=TRUE, warning=TRUE, error=TRUE, message=FALSE}

read_csv(here("data/SAFI_clean.csv"), na = "NULL") %>%
    pmap(function(items_owned, ...) str_count(items_owned,items)) %>%
    head(n = 2)
...
vector above. I could include `as_tibble_row()` in the anonymous function.
which would allow me to use `pmap_dfr()`, but I can also add steps to the 
pipleine, keeping each step in the pipeline a little simpler, which I prefer.
...
read_csv(here("data/SAFI_clean.csv"), na = "NULL")  %>%
  pmap(function(items_owned,...) str_count(items_owned,items)) %>%
  map(function(x) as_tibble_row(x,.name_repair = ~paste0("owns_",items))) %>%
  list_rbind()
...
read_csv(here("data/SAFI_clean.csv"), na = "NULL") %>%
  rowwise() %>%
  mutate(x = str_count(items_owned,items) %>%
                as_tibble_row(.name_repair = 
                                ~paste0("owns_",
                                        items))) %>%
  unnest(cols=c(x)) %>%
...
From the last data set, to convert all created variables to `NA` if `owns_NA = =1`, 
use [`mutate(across())`](https://dplyr.tidyverse.org/articles/colwise.html) to apply 
a function to all columns matching a tidy-select: 

```{r eval=TRUE, include = TRUE, echo=TRUE, warning=TRUE, error=TRUE, message=FALSE}

cleaned_with_separate_longer %>%
    mutate(across(c(starts_with("owns_"),-owns_NA), 
                  function(x) ifelse(owns_NA == 1,NA,x))) %>%
    select(owns_NA,starts_with("owns_"))
