...
separated. If there is no separator (for example your variables are called 
age1, age2, etc.) you'll have to use the`names_pattern` option.
...
wide_data %>%
    ...
    mutate(mean_age = mean(c_across(starts_with("age_")),
           ...
           num_women =  sum(c_across(starts_with("female_")),
                           na.rm=TRUE),
           hh_size = starts_with("female_") %>% c_across() %>% is.na(.) %>% `!` %>% sum()
          ) %>%
...
`%>% is.na(.) %>% `!` %>% sum()` is the same as `sum(!is.na())`, which simply counts the non-missing values.
