...
It uses `reorder` to put the longest lollipops are at the top
(though the grouping by villages messes this up), 
...
data %>%
    group_by(village, respondent_wall_type) %>%
    summarize(Count = n()) %>%
    mutate(respondent_wall_type = reorder(respondent_wall_type, Count)) %>%

    ggplot(aes(x = Count, y = village, group = respondent_wall_type)) +
        geom_linerange(aes(y = village, xmin = 0, xmax = Count), 
                       position = position_dodge(width = 0.5), color = "gray") +
        geom_point(aes(x = Count, y = village, color = respondent_wall_type), 
...
This plot is horrible for color blind people! Consider something like 
`[viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)`
to make more accessbile graphs.
