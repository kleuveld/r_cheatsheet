...
population <- declare_population(
  ...
  X = rnorm(N, mean = 20 , sd = 20),  # unobserved covariate
)

# Step 2: Define potential outcomes with unobserved characteristics
potential_outcomes <- declare_potential_outcomes(
  Y_Z_0 = 50 + 0.5 * education + 0.3 * gender + 0.1 * assets^2 + 0.2 * education * gender  + 0.6 * X + u,  # No intervention
  Y_Z_1 = 50 + 0.5 * education + 0.3 * gender + 0.1 * assets^2 + 0.2 * education * gender + 0.6 * X + u + 10  # Intervention increases fertilizer use by 10 units
)

# Step 3: Simulate treatment assignment with observed and unobserved characteristics
assignment <- declare_assignment(
  Z = 0.3 * education + 0.2 * gender + 0.01 * assets + 0.2 * X, # get a base number to base the treatment prob off
...
Then, I create a variable that's correlated to my outcome variable of interest, `food_secure`. 
I do this by simply regressing `food_secure` on some other things, and saving the error 
term.
...
model <- lm_robust(food_secure ~ HHsizeMAE + educated + LandOwned + NFertInput, data = rhomis)

rhomis <-
  rhomis %>%
  mutate(resid = food_secure - predict(model, rhomis))


```

I do something similar at village level:
...
rhomis_vill <-
  rhomis %>%
  group_by(village) %>%
  summarize(across(c(food_secure,HHsizeMAE, educated, LandOwned, NFertInput),
                   ~mean(.x,na.rm=TRUE)))

model_vill <- lm_robust(food_secure ~ HHsizeMAE + educated + LandOwned + NFertInput, data = rhomis_vill)

rhomis_vill <-
  rhomis_vill %>%
  mutate(resid_vill = food_secure - predict(model_vill, rhomis_vill)) %>%
  select(village,resid_vill)

```

Then, I put it in DeclareDesign.

I use dplyr to fabricate data, because I like it better.

Treatment assignment is as follows:
- Roughly 50% of the villages are selected, based on unobserved varaible `X_vill`
- Roughly 50% of the households are selected, based on unobserved varaible `X_hh`

Both these unobserved variables also affect outcomes.
...
population <- declare_population(
  rhomis %>% 
    left_join(
      rhomis_vill %>%
        mutate(
          treated_vill = correlate(given = resid_vill, draw_handler = draw_binomial, prob = 0.5 , rho = rho_vill )
        ),
      by = join_by(village)
    ) %>%
    mutate(
      # sampling
      treated_hh = correlate(given = resid, draw_handler = draw_binomial, prob = 0.5 , rho = rho_hh),
    ...
    filter(drop != 1) %>%
      ...
      do({
        if (unique(.$treated_vill) == 1) {
          resample_data(., N = c(village = clusters_treat, hh = hh_treat))
        } else {
          resample_data(., N = c(village = clusters_control_multi * clusters_treat, hh = hh_control_multi * hh_treat))
        }
      })
}
...
custom_match <- function(data) {
    ...
    # Fit propensity score model
    ps_model <- glm(Z ~ HHsizeMAE + educated + LandOwned + NFertInput, family = binomial, data = data)
...
First, let's examine bias, given household selection effects. 
We're assuming the village the village selection effect to be 0.
...
diagnosis %>%
  ...
  dplyr::select(rho_hh, bias = estimate, estimator) %>%
  ggplot(aes(x = rho_hh, y = bias, 
...
Matching on criteria unrelated to the selection process doesn't help reduce bias.

Then, let's examine power. Note that this takes a while!
...
diagnosis <-
  design %>%
  redesign(clusters_control = 4:7 * 10,
           effect = 0.05 * 1:5) %>%
  diagnose_design(sims = 200) %>%
  tidy() %>%
  as_tibble()
...
diagnosis %>%
  filter(diagnosand == "power") %>%
  dplyr::select(clusters_control, power = estimate, estimator, effect) %>%
  ggplot(aes(x = clusters_control, y = power, 
             shape = estimator, color=estimator)) +
    geom_line() +
    geom_point() +
    geom_hline(yintercept=0.8,linetype=2) +
