...
Treatment then is treated households in treated villages. 
...
- I drop any untreated household in treated villages; this is a fairly common setup.
...
diagnosis <-
  design %>%
  redesign(clusters_treat = 3:6 * 10,
           effect = 0.05 * (1:3)^2) %>%
  diagnose_design(sims = 200) %>%
  tidy() %>%
  as_tibble()
