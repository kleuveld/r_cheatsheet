...
I could have declared a test here using `declaretest()`, which has 
the same syntax.
...
```
Then I use the fixest package to estimate a Two-way fixed effects (2FE) model. 
This is equivalent to the model above for this case, but the 
2FE can be more easily extended to more complex designs  
([but beware!](https://doi.org/10.1017/pan.2020.33)).

I also define a custom function to summarize the results. DeclareDesign
uses `tidy()` to summarize outputs, but expects
a column in its output with the outcome variable. `feols()` doesn't
create that column, which makes some output of Declare Design look a
bit inconsistent. So I create `add_Y_to_feols()` to do so. 

```{r eval=TRUE, include = TRUE, echo=TRUE, warning=TRUE, error=TRUE, message=FALSE}

library(fixest)

add_Y_to_feols <- function(regoutput) {
  #function that takes the regression output of feols
  #and returns a tidy data frame with its results
  #plus an "outcome" column with "Y" in it
...
As third estimator, I use a custom estimation function. I use
the lmtest/sandwich way of estimating robust standard errors.
I use HC1 standard errors, as that's equivalent to the SEs I used
above, but I make it possible to change the type easily.
...
custom_robust <- function(formula, data, type="HC1"){
  ...
  lm_robust <- coeftest(lm, vcov = vcovHC(lm, type=type))
  lm_robust %>% 
    broom::tidy(conf.int=TRUE) %>%
    mutate(outcome = "Y") %>%
...
Note that the results of my three estimators are identical.
...
Looks good! But what if I was too optimistic? Using the 
