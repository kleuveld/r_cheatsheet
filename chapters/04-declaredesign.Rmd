...
population <- declare_population(
  rhomis %>% 
    left_join(
      ...
      unique() %>%
      mutate(
        # generate an indicator for a treated village
        X_vill =  rnorm(nrow(.), sd = 2),
        prob_treat_vill = plogis(X_vill - mean(X_vill) + 0.5), #set the mean so that about 50% of villages are treated
        treated_vill = rbinom(nrow(.),1,prob_treat_vill) # draw based on probabilities.
      )
    ) %>%
    mutate(
      # sampling
      X_hh =  rnorm(nrow(.), sd = 2),
      prob_treat_hh = plogis(X_hh - mean(X_hh) + 0.5), #set the mean so that about 50% of villages are treated
      treated_hh = rbinom(nrow(.),1,prob_treat_hh), # draw based on probabilities.

      # boost food security with X_hh and X_vill
      prob_boost = plogis((X_hh - mean(X_hh)) + (X_vill - mean(X_hh)) + 0.1),
      food_secure = pmin(1,food_secure + rbinom(nrow(.),1,prob_boost)),
...
filter_obserable_hhs <- function(data){
    ...
    filter(drop != 1) %>%
      ...
      do({
        if (unique(.$treated_vill) == 1) {
          resample_data(., N = c(village = 50, hh =15))
        } else {
          resample_data(., N = c(village = 50, hh = 30))
        }
      })
}

sampling <- declare_step(
  handler = filter_obserable_hhs
)
...
design <-
  population +  inquiry + assignment +  measurement + ols_estimator
