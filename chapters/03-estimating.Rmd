...
On a serious note: having R do it does make for more reproducible code, 
...
function. This doesn't report the number of 
clusters, but it is saved in the `lm_robust()` object.
...
# coeftest object
# note that it expects cluster to be provided as a string containing the var name
glm_robust <- function(formula, family, data, cluster, type = "CR2") {

  # Estimate model
  fit <- glm(formula, family = family, data = data)

  # use black magic to get the vector of clusters
  cluster_vec <- data[[cluster]][as.integer(rownames(model.frame(fit)))]
  ...
  # Coefficient test
  ct <- lmtest::coeftest(fit, vcov. = vcov_cr)

  # Attach useful attributes
  # these can then be read by other functions, e.g., glance()
  attr(ct, "vcov")      <- vcov_cr
  attr(ct, "nclusters") <- length(unique(cluster_vec))
  attr(ct, "model")     <- fit
  attr(ct, "nobs")      <- stats::nobs(fit)
  attr(ct, "logLik")    <- stats::logLik(fit)

  # set the class of my object: this controls what methods are used to extract info
  class(ct) <- c("glm_robust")
  ct
}
...
glance.glm_robust <- function(x, ...){
  tibble::tibble(
    ...
    clustered = if_else(!is.null(nclusters), "Yes", "No")
  )
}
...
probitrobust <- 
  glm_robust(uses_fertilizer ~ treatment + educated,
             family = binomial(link = "probit"),
             data = data,
             cluster = "village", # provide it as a string!
             type = "CR2")

broom::glance(probitrobust)
...
And you can simply reuse these functions in future projects!
...
ame_robust <- function(x) {
  ...
  # save clusters and observations as attributes
  attr(ame, "nclusters") <- attr(x, "nclusters")
  attr(ame, "nobs")      <- attr(x, "nobs")

  class(ame) <- c("ame_robust", class(ame))
  ame
}
...
- The plot is contained in a reusable function.
