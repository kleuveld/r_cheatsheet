...
a path for me. This library is great for shared projects!
...
write2word(descriptive_table, here("tables/summ_stats.docx"), title = "Descripte Statistics", quiet = TRUE)
...
lm_robust <- coeftest(lm, vcov = vcovHC(lm, type = 'HC0',cluster=~village))
lm_robust
...
probit_robust <- coeftest(probit, vcov = vcovHC(probit, type = 'HC0',cluster=~village))
probit_robust
...
lm_robust <- 
  tidy_override(lm_robust,
                glance = list(n_clusters = 4),
                extend=TRUE)
probit_robust <- 
  tidy_override(probit_robust,
                glance = list(n_clusters = 4),
                extend=TRUE)
#make the table
reg_table <-
  huxreg("Linear" = lm,"Linear Robust" = lm_robust,
         "Probit" = probit, "Probit Robust" = probit_robust,
         statistics = c(N = "nobs",
                        Clusters = "n_clusters", 
                        "Adj. R2" = "adj.r.squared")) %>% 
  set_number_format(row=9,value=0)
